...
現行実装との差分は実装工程で確認し、この文書を基準に必要な箇所だけ変更します。
...
この文書の作成だけでは既存リンクを変更しません。

実装時は以下の順序で行います。

1. 現在実際に存在する外部流入リンクを洗い出す
2. このマスターと照合する
3. 必要なリンクだけUTMを付与・修正する
4. GA4リアルタイムで流入値を確認する
5. 既存CTA・フォーム・301転送・内部リンクを壊していないことを確認する

未登録のランサーズ、ココナラ、ジモティーや、まだ営業開始していない業種には先行実装しません。
